...
export const Dock = ({ children, magnification = 35, position = 'bottom', size = 50 }: DockProps) => {
  ...
  return (
    <MouseProvider>
      <DockContext.Provider value={{ dockPosition: position, hovered, magnification, size, width }}>
        <ul className={`${styles.stack} ${styles[position]}`} onMouseOut={() => setHovered(false)} onMouseOver={() => setHovered(true)} ref={ref}>
          {children}
        </ul>
